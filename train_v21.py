"""
FLM V21 — Lean EN→EN Concept Autoencoder + Concept Whitening
================================================================================
Single EN decoder (3L), concept whitening bottleneck, massive diverse data.
No translation heads, no auxiliary geometry losses.
Structure emerges from data diversity + tight bottleneck + whitening constraint.

Architecture: 6L encoder (384d) → 32×16=512 bottleneck + whitening → 3L decoder (384d)
Data: Wikipedia + Gutenberg + StackExchange + arXiv + USGPO (~300GB, streaming)

Usage:
    python train_v21.py --fresh            # start from scratch
    python train_v21.py                    # auto-resume
    python train_v21.py --eval-only        # diagnostics only
"""
...
from concept_model import (ConceptConfig, ConceptAutoencoderV21,
                           reconstruction_loss,
...
CHECKPOINT_DIR = "checkpoints/concept_v21"
LOG_DIR = "logs"
...
BATCH_SIZE = 64             # up from 24 — single head, no aux losses
PEAK_LR = 3e-4             # slightly higher for faster convergence
MIN_LR = 1e-5
WARMUP_STEPS = 2000
TOTAL_STEPS = 600_000
WEIGHT_DECAY = 0.01
...
LOG_PATHS = {
    "log": f"{LOG_DIR}/concept_v21.log",
    "metrics": f"{LOG_DIR}/concept_v21_metrics.csv",
}
...
class StreamingPretrainDataset:
    ...
    def _chunk_text(self, text):
        ...
        ~4 chars per mBERT token on average."""
        max_chars = self.max_len * 4  # rough estimate
...
def load_checkpoint(path, device="cuda"):
    log(f"Loading V21 checkpoint from {path}...")
    ...
    if isinstance(cfg, dict):
        ...
        config = cfg
    model = ConceptAutoencoderV21(config).to(device)
    ...
    log(f"Resumed V21: {total:,} params | step {step} | em_ema={exact_match_ema:.3f}")
...
def train(resume_from=None, fresh=False, eval_only=False):
    ...
    for step in range(start_step, TOTAL_STEPS):
        ...
        if (step + 1) % LOG_EVERY == 0:
            ...
            log(f"step {step+1:>7d} [V21] | en={avg_loss:.4f} | "
