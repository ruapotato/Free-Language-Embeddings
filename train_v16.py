"""
FLM V16 — Lean Hydra + Programmatic Geometry
================================================================================
Key changes from V15:
  - 3 decoder heads: EN recon, EN paraphrase, Semantic parse (dropped FR/ES)
  ...
  - Fresh weights only (no V15 checkpoint loading)
...
MODEL_CONFIG = dict(
    ...
    fr_vocab_size=1,        # unused placeholder (config requires it)
    es_vocab_size=1,        # unused placeholder
    enc_hidden=384,
    enc_layers=6,
    enc_heads=6,
...
DATA_WEIGHTS = {
    "para": 0.50,      # paraphrase pairs — key for concept learning
    "parse": 0.50,     # semantic parse — structured output
}
...
class MultiSourceDataset:
    """Loads para + parse data sources for the lean hydra decoder heads."""
    ...
    def get_batch(self, batch_size, head):
        ...
        tgt_enc = self.en_tok(tgt_texts, max_length=self.max_len,
                              padding=True, truncation=True, return_tensors="pt")
...
def train(resume_from=None, fresh=False, eval_only=False):
    ...
    log(f"  Heads: EN({config.dec_layers}L) PARA({config.dec_layers}L) "
        f"PARSE({config.dec_layers}L)")
