...
**V33 — Mixed Skip-Gram + CBOW word2vec** (training)

Combining both word2vec objectives on shared embeddings: skip-gram and CBOW alternate steps, giving the model two views of the same co-occurrence data. A/B comparison against V28's 43.9% on the Google analogy benchmark.

60M params, 1M steps, 17.7 step/s on RTX 3090.
...
| Word & path embeddings | [V28-V33](docs/history_embeddings.md) | Word2vec (300d, 3D), path2vec on Debian filesystem, mixed SG+CBOW | Whole-word vocab is critical; 3D captures regions but not individuals; filesystem = hierarchy not analogy |
...
4. **Small models can compress well.** 25.9M params outperforms 248M when the architecture is right.
