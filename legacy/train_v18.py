"""
FLM V18 — V15 architecture + programmatic geometry data + geo frequency ramp
================================================================================
V15 architecture (5 heads: EN recon, FR/ES trans, Paraphrase, Semantic Parse)
with 6L decoders (up from 4L in V15). Bottleneck: 32×16 = 512 dims.

Key changes from V15:
  - 6-layer decoders (was 4L in V15)
  - Geometry data from GeometryDataGenerator (programmatic, train/test split)
    instead of hardcoded constants — enables genuine generalization measurement
  - Geo frequency ramp: every step for first 2K steps after gate, then ramp
    to every-5 over 10K steps (was fixed every-5 in V15)
  - Geometry probing uses TEST split for honest generalization metrics
  - All geometry helper functions encode via bottleneck directly

Same model class (ConceptAutoencoderV14), same loss functions, same gate logic.

Usage:
    python train_v18.py --fresh            # start from scratch
    python train_v18.py                    # auto-resume
    python train_v18.py --eval-only        # diagnostics only
"""
...
CHECKPOINT_DIR = "checkpoints/concept_v18"
LOG_DIR = "logs"
...
MODEL_CONFIG = dict(
    ...
    dec_hidden=384,
    dec_layers=6,           # 6 layers per head (up from 4 in V15)
    dec_heads=6,
...
LOG_PATHS = {
    "log": f"{LOG_DIR}/concept_v18.log",
    "metrics": f"{LOG_DIR}/concept_v18_metrics.csv",
}
...
# ---------------------------------------------------------------------------
# Geometry batch generation (using GeometryDataGenerator)
# ---------------------------------------------------------------------------
...
def load_checkpoint(path, device="cuda"):
    log(f"Loading V18 checkpoint from {path}...")
    ...
    log(f"Resumed V18: {total:,} params | step {step} | em_ema={exact_match_ema:.3f}")
...
def train(resume_from=None, fresh=False, eval_only=False):
    ...
    log("=" * 70)
    log("FLM V18 — V15 ARCH + PROGRAMMATIC GEO + GEO FREQ RAMP")
    log("  Bottleneck: 32x16 = 512 dims | 6L decoders")
    log("  Heads: EN recon | FR trans | ES trans | EN para | Semantic parse")
    log("  Geometry: from GeometryDataGenerator (train/test split)")
    log("  Geo freq: every step for 2K, ramp to 1/5 over 10K")
    log("=" * 70)
    ...
    log(f"\nTraining plan (V18 — V15 arch + programmatic geo):")
    log(f"  Model: {total:,} params ({total/1e6:.1f}M)")
    ...
    for step in range(start_step, TOTAL_STEPS):
        ...
        with torch.amp.autocast("cuda", dtype=torch.float16):
            ...
            # --- Geometry losses (recon-gated, frequency ramp) ---
            ...
            # Frequency ramp: every step for 2K, then ramp to every-N over 10K
            if geo_scale > 0 and geo_gate_step >= 0:
                steps_since_gate = step - geo_gate_step
            ...
            if run_geo:
                ...
                c_a, c_b, c_c, c_d = get_analogy_batch(
                    train_geo_gen, geo_tokenizer, model, device, batch_size=6)
                ...
                dir_groups = get_direction_batch(
                    train_geo_gen, model, geo_tokenizer, device)
        ...
        if (step + 1) % EVAL_EVERY == 0:
            ...
            log(f"  GEOMETRY (TEST): analogy={geo['analogy_avg']:.3f} "
                ...
                f"dir_con={geo.get('dir_consistency', geo.get('direction_consistency', 0)):.3f} "
            ...
            log(f"  HEAD DIST: {dist} | geo_scale={geo_scale:.2f} "
                f"geo_freq=1/{geo_every_n} gate_step={geo_gate_step}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train concept autoencoder V18 (V15 arch + programmatic geo + geo freq ramp)")
