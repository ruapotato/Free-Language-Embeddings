"""
FLM V22 — Scaled EN→EN Concept Autoencoder + Concept Whitening
================================================================================
English-only tokenizer (BERT-base-uncased, 30K vocab) + scaled model (~210M params).
Same philosophy as V21: structure from compression of diverse data, not auxiliary losses.

Architecture: 12L encoder (768d) → 32×16=512 bottleneck + whitening → 8L decoder (768d)
Data: Wikipedia + Gutenberg + StackExchange + arXiv + USGPO (~300GB, streaming)

Key changes from V21:
- English-only tokenizer: 30K vocab (down from 119K mBERT) — 4x easier reconstruction
- Scaled model: 768d hidden, 12L enc, 8L dec → ~210M params (up from 117M)
- Batch 64 — 248M model fits at ~15GB on RTX 3090 (headroom for torch.compile)

Usage:
    python train_v22.py --fresh            # start from scratch
    python train_v22.py                    # auto-resume
    python train_v22.py --eval-only        # diagnostics only
"""
...
from concept_model import (ConceptConfig, ConceptAutoencoderV22,
                           reconstruction_loss,
...
CHECKPOINT_DIR = "checkpoints/concept_v22"
LOG_DIR = "logs"

MODEL_CONFIG = dict(
    vocab_size=30522,       # bert-base-uncased (English-only, 4x smaller than mBERT)
    enc_hidden=768,
    enc_layers=12,
    enc_heads=12,
    enc_intermediate=3072,
    num_concepts=32,
    concept_dim=16,         # 32×16 = 512 bottleneck (same as V21)
    dec_hidden=768,
    dec_layers=8,
    dec_heads=12,
...
BATCH_SIZE = 64             # 248M model fits at ~15GB with batch 64 (torch.compile needs headroom)
PEAK_LR = 2e-4             # slightly lower for larger model stability
MIN_LR = 1e-5
WARMUP_STEPS = 2000
TOTAL_STEPS = 600_000
WEIGHT_DECAY = 0.01
...
LOG_PATHS = {
    "log": f"{LOG_DIR}/concept_v22.log",
    "metrics": f"{LOG_DIR}/concept_v22_metrics.csv",
}
...
class StreamingPretrainDataset:
    ...
    def _chunk_text(self, text):
        ...
        max_chars = self.max_len * 5  # rough estimate (~5 chars per BERT-uncased token)
...
def load_checkpoint(path, device="cuda"):
    log(f"Loading V22 checkpoint from {path}...")
    ...
    if isinstance(cfg, dict):
        ...
        config = cfg
    model = ConceptAutoencoderV22(config).to(device)
    ...
    log(f"Resumed V22: {total:,} params | step {step} | em_ema={exact_match_ema:.3f}")
...
def train(resume_from=None, fresh=False, eval_only=False):
    ...
    log("=" * 70)
    log("FLM V22 — SCALED EN→EN + EN-ONLY VOCAB + CONCEPT WHITENING")
    ...
    log(f"\nTraining plan (V22 — Scaled EN→EN):")
    log(f"  Model: {total:,} params ({total/1e6:.1f}M)")
    ...
    for step in range(start_step, TOTAL_STEPS):
        ...
        if (step + 1) % LOG_EVERY == 0:
            ...
            log(f"step {step+1:>7d} [V22] | en={avg_loss:.4f} | "
