"""
FLM V9 — Recon-Gated Slot Specialization
================================================================================
Key changes from V8:
  1. RECON-GATED GEOMETRY: geometry losses suppressed until reconstruction works.
     geo_scale ramps 0→1 as recon EMA drops from 0.5→0.1. This ensures concept
     vectors actually encode meaningful information before geometry pushes them.
  2. DYNAMIC PHASES (driven by recon quality, not fixed step counts):
     - Phase 1: Recon focus. geo_scale < 1.0, decoder gets 3x weight boost.
     - Phase 2: Slot focus. Starts when geo_scale reaches 1.0 (recon < 0.1).
       Cycles 500 steps per slot, all 32 slots.
     - Phase 3: Joint fine-tune. All slots equal, remaining steps.
  3. Recon boost when gate is low: dec_loss *= 1 + 2*(1-geo_scale) — up to 3x.

V8 post-mortem: slot structure excellent (31/32 assigned, iso=0.554) but
reconstruction was garbage (gibberish output despite 0.55 teacher-forced CE).
Clustering gap still tiny (+0.018). Geometry pushed vectors around but decoder
couldn't use them — concept space may not encode real information.

Usage:
    python train_v9.py --fresh          # start from scratch
    python train_v9.py --resume         # resume from V9 checkpoint
    python train_v9.py --eval-only      # diagnostics only
"""
...
from concept_model import (ConceptConfig, ConceptAutoencoder,
                           ...
                           batch_repulsion_loss, hard_repulsion_loss)

# ---------------------------------------------------------------------------
# Config
# ---------------------------------------------------------------------------

CHECKPOINT_DIR = "checkpoints/concept_v9"
LOG_DIR = "logs"
...
HARD_REPULSION_WEIGHT = 5.0    # V9: targets worst-offending pairs for clustering gap
...
# Recon-gated geometry: don't push geometry until decoder works
# geo_scale ramps linearly from 0.0 at RECON_GATE_CEILING to 1.0 at RECON_GATE_TARGET
RECON_GATE_TARGET = 0.1       # geometry fully active below this recon loss
RECON_GATE_CEILING = 0.5      # geometry fully suppressed above this
RECON_EMA_DECAY = 0.99        # smoothing for recon loss tracking

# Gradient taper (how much recon gradient reaches encoder through bottleneck)
TAPER_BOTTLENECK = 0.3         # will be boosted to 1.0 during Phase 1 (recon focus)
...
# ===================== V8: PHASE SCHEDULE =====================
# Phase 1: Recon Focus — geo gate suppresses geometry until recon < 0.1
#           (no fixed step count — transitions when geo_scale reaches 1.0)
#
# Phase 2: Slot Focus — cycle through each slot with gradient emphasis
#           (starts automatically when geo gate opens)
STEPS_PER_SLOT = 500            # steps focusing on each slot
PHASE2_TOTAL_SLOTS = 32

# Phase 3: Joint Fine-tune — all slots equal, remaining steps
#           (starts after all 32 slots have been cycled)
...
LOG_PATHS = {
    "log": f"{LOG_DIR}/concept_v9.log",
    "metrics": f"{LOG_DIR}/concept_v9_metrics.csv",
}
...
def get_slot_scales(phase, focus_slot, device):
    """Return per-slot gradient scale tensor for current phase/slot."""
    if phase == 2 and 0 <= focus_slot < 32:
...
def measure_geometry(model, tokenizer, device="cuda"):
    ...
    # Use a deep copy of the tokenizer to avoid "Already borrowed" race
    # condition with prefetch threads that also call the tokenizer.
    tok = copy.deepcopy(tokenizer)
    model.eval()
    m = _unwrap(model)

    def encode_texts(texts):
        enc = tok(texts, max_length=64, padding=True,
                  truncation=True, return_tensors="pt").to(device)
...
def save_checkpoint(model, classifiers, optimizer, scaler, config,
                    ...
                    phase=1, focus_slot=-1, phase2_slot_step=0, recon_ema=0.5):
    ...
    ckpt = {
        ...
        "phase2_slot_step": phase2_slot_step,
        "recon_ema": recon_ema,
    }
...
def load_checkpoint(path, axis_dataset, device="cuda"):
    ...
    phase_state = {
        ...
        "focus_slot": ckpt.get("focus_slot", -1),
        "phase2_slot_step": ckpt.get("phase2_slot_step", 0),
        "recon_ema": ckpt.get("recon_ema", 0.5),
    }
    return model, classifiers, optimizer, scaler, config, step, phase_state
...
def train(resume_from=None, fresh=False, from_v7=False, eval_only=False):
    ...
    log("=" * 70)
    log("FLM V9 — RECON-GATED SLOT SPECIALIZATION")
    log("=" * 70)
    ...
    phase_state = None  # will be set if resuming
    if resume_from:
        model, classifiers, optimizer, scaler, config, start_step, phase_state = \
    ...
    log(f"\nTraining plan (V9 — Recon-Gated Slot Specialization):")
    ...
    log(f"  V9 PHASES: P1=ReconFocus(until recon<{RECON_GATE_TARGET}) -> "
        f"P2=SlotFocus({PHASE2_TOTAL_SLOTS}x{STEPS_PER_SLOT}/slot) -> P3=JointFinetune(rest)")
    log(f"  Phase 2 scales: focus={SLOT_FOCUS_SCALE} background={SLOT_BACKGROUND_SCALE}")
    log(f"  Hard repulsion weight: {HARD_REPULSION_WEIGHT}")
    ...
    log(f"  Recon gate: geo OFF above recon={RECON_GATE_CEILING}, "
        f"fully ON below recon={RECON_GATE_TARGET}, EMA decay={RECON_EMA_DECAY}")
    ...
    # Dynamic phase state (driven by recon gate, not fixed steps)
    # Phase 1: recon focus (geo gate < 1.0)
    # Phase 2: slot-by-slot (geo gate reached 1.0, cycling through slots)
    # Phase 3: joint fine-tune (all slots done)
    if phase_state:
        phase = phase_state["phase"]
        focus_slot = phase_state["focus_slot"]
        phase2_slot_step = phase_state["phase2_slot_step"]
        recon_ema = phase_state["recon_ema"]
        log(f"  Restored phase state: P{phase} slot={focus_slot} recon_ema={recon_ema:.3f}")
    else:
        phase = 1
        focus_slot = -1
        phase2_slot_step = 0
    ...
    prev_phase_logged = -1
    prev_focus_logged = -1
    geo_scale = max(0.0, min(1.0,
        (RECON_GATE_CEILING - recon_ema) / (RECON_GATE_CEILING - RECON_GATE_TARGET)))

    for step in range(start_step, TOTAL_STEPS):
        if shutdown_requested:
            break

        # Dynamic phase management (driven by recon gate)
        if phase == 1 and geo_scale >= 1.0:
            # Recon is good — start slot-by-slot
            phase = 2
            focus_slot = 0
            phase2_slot_step = 0
        elif phase == 2:
            phase2_slot_step += 1
            if phase2_slot_step >= STEPS_PER_SLOT:
                focus_slot += 1
                phase2_slot_step = 0
                if focus_slot >= PHASE2_TOTAL_SLOTS:
                    phase = 3
                    focus_slot = -1
        # If recon drifts bad during P2/P3, geo_scale handles it (soft gate)
        # — we don't revert to P1, just reduce geometry influence

        slot_scales = get_slot_scales(phase, focus_slot, device)

        # Log phase transitions
        if phase != prev_phase_logged or (phase == 2 and focus_slot != prev_focus_logged):
            if phase == 1:
                log(f"  PHASE 1: Recon focus (geo_scale={geo_scale:.2f}, "
                    f"waiting for recon < {RECON_GATE_TARGET})")
            elif phase == 2:
            ...
            prev_phase_logged = phase
            prev_focus_logged = focus_slot
        ...
        with torch.amp.autocast("cuda", dtype=torch.float16):
            ...
            # Hard repulsion (V9: target worst offenders for clustering gap)
            hrepul_loss, hrepul_max = hard_repulsion_loss(concepts, target_sim=REPULSION_TARGET)
            geo_loss = geo_loss + HARD_REPULSION_WEIGHT * hrepul_loss
        ...
        hrepul_val = hrepul_loss.item()
        hrepul_max_val = hrepul_max
        ...
        with torch.amp.autocast("cuda", dtype=torch.float16):
            ...
            # Phase 1: full gradient through bottleneck for faster recon learning
            # Phase 2/3: taper down so geometry owns the encoder
            taper = 1.0 if phase == 1 else TAPER_BOTTLENECK
            concepts_for_dec = gradient_taper(concepts_raw, taper)
        ...
        # =================================================================
        # RECON-GATED GEOMETRY + COMBINED BACKWARD
        # =================================================================

        # Update recon EMA and compute geometry gate
        recon_ema = RECON_EMA_DECAY * recon_ema + (1 - RECON_EMA_DECAY) * r_loss_val
        geo_scale = max(0.0, min(1.0,
            (RECON_GATE_CEILING - recon_ema) / (RECON_GATE_CEILING - RECON_GATE_TARGET)))

        # Boost recon weight when geo gate is low (decoder needs more focus)
        recon_boost = 1.0 + 2.0 * (1.0 - geo_scale)  # 3x when geo=0, 1x when geo=1
        total_loss = geo_scale * geo_loss + recon_boost * dec_loss
        ...
        if (step + 1) % LOG_EVERY == 0:
            ...
            log(f"step {step+1:>7d} | loss {avg_loss:.4f} "
                ...
                f"repul={repul_val:.3f} hrepul={hrepul_val:.3f}) | "
                f"p_sim={p_sim_val:.3f} n_sim={n_sim_val:.3f} "
                f"wo_sim={wo_sim_val:.3f} sp_sim={slot_para_sim:.3f} "
                f"r_sim={repul_sim_val:.3f} hr_max={hrepul_max_val:.3f} "
                f"cls_acc={cls_acc_val:.3f} | "
                f"geo={geo_scale:.2f} lr {current_lr:.2e} | {phase_tag} {pct:.1f}%")
        ...
        if (step + 1) % CHECKPOINT_EVERY == 0:
            ...
            save_checkpoint(model, classifiers, optimizer, scaler, config,
                            ...
                            phase=phase, focus_slot=focus_slot,
                            phase2_slot_step=phase2_slot_step, recon_ema=recon_ema)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train concept autoencoder V9")
