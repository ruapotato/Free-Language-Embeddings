...
NCE_WEIGHT = 3.0              # cranked up — must push unrelated apart
HARD_NEG_WEIGHT = 0.5
WO_WEIGHT = 1.5               # word order needs more pressure
DECORR_WEIGHT = 1.0
STS_WEIGHT = 1.0              # graded sim helps calibrate distances
SLOT_ISO_WEIGHT = 1.0         # already learned well at 2.0, can relax
AXIS_BATCH_SIZE = 128         # doubled — we have 2.1M axis pairs now
...
class ConceptAxisDataset:
    ...
    def _load(self):
        ...
        # Build sqrt-proportional sampling weights so small slots aren't drowned
        ...
        non_empty = len(self.active_slots)
        log(f"  Active slots: {non_empty}/32")
        # Show per-slot data balance
    ...
    def get_batch(self, batch_size):
        """Get a random batch with sqrt-proportional slot sampling. Returns (enc_base, enc_var, slot_ids)."""
        bases, variants, slot_ids = [], [], []
        # Sample slots using sqrt-proportional weights (small slots get boosted)
...
def hard_negative_info_nce(concepts_pos_a, concepts_pos_b,
    ...
    # Slot-aware sim for positive pairs: (B_pos, B_pos)
    sim_pos_raw = slot_similarity_matrix(concepts_pos_a, concepts_pos_b)

    if concepts_hn_a is not None:
        # Concat hard negatives as extra columns
        hn_all = torch.cat([concepts_hn_a, concepts_hn_b], dim=0)  # (2*B_hn, K, D)
        sim_hn_raw = slot_similarity_matrix(concepts_pos_a, hn_all)  # (B_pos, 2*B_hn)
...
def sts_loss(concepts_a, concepts_b, target_scores):
    ...
    sim_matrix = slot_similarity_matrix(concepts_a, concepts_b)  # (B, B)
    pred_sim = sim_matrix.diag()  # (B,) — paired similarities
